...
This allows us to narrow in on specific points on the map which can give us a more detailed picture.
...
starbucks_with_2018_pop_est %>% 
  ...
  labs(title = "Starbucks in the United States",
       caption = "Plot by Rayyan Mobarak") +
  theme(legend.background = element_blank()) + 
  theme_map() +
...
```{r}
favorite_places_Rayyan <- tibble(
  place = c("Macalester College","Aldi's","My Apartment","Cadenza Music","Mall of America","Buffalo Wild Wings","Caspian Bistro","University of Minnesota","Wellesley College","Wakame Sushi & Asian Bistro"),
  long = c(-93.1672242,-93.2465051,-93.1565237,-93.1672612,-93.2421282,-93.2407163,-93.2220261,-93.1284640,-71.3132515,-93.321944),
  lat = c( 44.9412279,45.0599317, 44.9391414, 44.9460000,44.8533668, 44.8536091,44.9734543, 44.7395401, 42.2920042, 44.947059),
  favorite_place = c("Yes", "No", "Yes", "Yes", "No", "No", "No", "No", "No", "No")
  )
```
...
pal <- colorFactor(
  ...
  domain = favorite_places_Rayyan$favorite_place)
leaflet(data = favorite_places_Rayyan) %>% 
...
It seems that casual rider have a high percentage of renting their bikes out from stations in downtown where there could be some good scenery.
...
We can see that the cases have gone up over time across the 4 maps.
...
leaflet(data = MplsStops, 
        options = leafletOptions(preferCanvas = TRUE)) %>% 
  addProviderTiles(providers$Stamen.TonerHybrid,
                   options = providerTileOptions(updateWhenZooming = FALSE,updateWhenIdle = FALSE))  %>% 
  addCircles(lng = ~long, 
             lat = ~lat, 
             label = ~preRace, 
             weight = 10, 
  ...
  addLegend(pal = pal, 
            values = ~preRace, 
            opacity = .5, 
            title = "Race of the person before conversation",
            position = "bottomright") 
```
